...
class RoomGraphics:
    ...
    def __init__(self):
        ...
        self.game = g.Game()
        self.game.add_to_map(self.game.main_room, "east", self.game.room_class.all_rooms.get("Vestibule"))
        self.run_graphics()
            ...
            for y in range(len(self.game.map)):
                for x in range(len(self.game.map[y])):
                    if self.game.map[y][x] is not None:
                        ...
                        img = pg.transform.smoothscale(img, (100, 100))
                        self.game_display.blit(img, (((x - self.game.main_room[0]) * 100) + self.shift_x,
